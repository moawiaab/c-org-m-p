<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group row {{ $errors->has('expense.budget_id') ? 'invalid' : '' }}">
        <label class="control-label col-sm-2 required" for="budget">{{ trans('cruds.expense.fields.budget') }}</label>
        <div class="col-sm-8">
            <x-select-list class="form-control" id="budget" name="budget" :options="$this->listsForFields['budget']"
                wire:model="expense.budget_id" required />
    ...
    <div class="form-group row {{ $errors->has('expense.amount') ? 'invalid' : '' }}">
        <label class="control-label col-sm-2 required" for="amount">{{ trans('cruds.expense.fields.amount') }}</label>
        <div class="col-sm-8">
    ...
    <div class="form-group row {{ $errors->has('expense.text_amount') ? 'invalid' : '' }}">
        <label class="control-label col-sm-2 required" for="text_amount">{{ trans('cruds.expense.fields.text_amount')
            }}</label>
        <div class="col-sm-8">
    ...
    <div class="form-group row {{ $errors->has('expense.beneficiary') ? 'invalid' : '' }}">
        <label class="control-label col-sm-2 required" for="beneficiary">{{ trans('cruds.expense.fields.beneficiary')
            }}</label>
        <div class="col-sm-8">
            <input class="form-control" type="text" name="beneficiary" id="beneficiary" required
                wire:model.defer="expense.beneficiary">
            <div class="validation-message">
    ...
    <div class="form-group row {{ $errors->has('expense.details') ? 'invalid' : '' }}">
        <label class="control-label col-sm-2 required" for="details">{{ trans('cruds.expense.fields.details') }}</label>
        <div class="col-sm-8">
